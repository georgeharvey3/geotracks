...
function App() {
    ...
    const selectSong = () => {
        ...
        const newAlbums = albums.filter(
            (_, index) => index !== albumIndexChoice
        );
    ...
    const onFormSubmit = (e) => {
        ...
        if (countryAnswer) {
            const guessedCountry = countriesJSON.find(
                (country) =>
                    country.name.toLowerCase() === countryAnswer.toLowerCase()
            );
            if (guessedCountry) {
                setSubmitted(true);
                setErrorMessage("");
                ...
                if (
                    countryAnswer.toLowerCase() === song.country.toLowerCase()
                ) {
                    setGuesses([
                        ...guesses,
