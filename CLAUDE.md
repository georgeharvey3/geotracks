...
- **The node is write-only from the client.** `suggestions` gets no `.read` in `database.rules.json` and inherits `false` from the root, so the only readers are the Firebase console and the project owner's Firebase CLI — the same privilege, which is what the accept script reads through. Abuse is met with a submit-button guard for now; **if junk appears, the response is Firebase App Check (reCAPTCHA v3)** — decided in advance so it is executed rather than invented in a panic.
