...
const createDefaultProps = (overrides: Partial<React.ComponentProps<typeof Game>> = {}) => ({
  ...
  submitted: false,
  guesses: [] as Guess[],
  onNextSongClicked: jest.fn(),
  song: { country: 'France', link: 'https://open.spotify.com/track/abc', album: 'Test Album' } as Song,
...
describe('Game', () => {
  it('renders the question prompt', () => {
    ...
    expect(screen.getByText('Which country does this song originate from?')).toBeInTheDocument();
  });

  it('renders play button when song is ready and not playing', () => {
    render(<Game {...createDefaultProps({ songReady: true, songPlaying: false })} />);
    expect(screen.getByTestId('PlayArrowIcon')).toBeInTheDocument();
  });

  it('renders pause button when song is playing', () => {
    render(<Game {...createDefaultProps({ songReady: true, songPlaying: true })} />);
    expect(screen.getByTestId('PauseIcon')).toBeInTheDocument();
  });

  it('renders replay button when song is finished', () => {
    render(<Game {...createDefaultProps({ songReady: true, songFinished: true })} />);
  ...
  it('shows Continue when turnsRemaining is 0', () => {
    render(<Game {...createDefaultProps({ finished: true, turnsRemaining: 0 })} />);
  ...
  it('shows error message when present', () => {
    render(<Game {...createDefaultProps({ errorMessage: "Unrecognised country: 'XYZ'" })} />);
  ...
  it('shows CurrentScore in competition mode', () => {
    render(<Game {...createDefaultProps({ isCompetition: true, turnsRemaining: 8, score: 230 })} />);
  ...
  it('shows GeoHints half points warning in competition mode', () => {
    ...
    expect(screen.getByText('Enabling GeoHints will score half points')).toBeInTheDocument();
  });

  it('hides GeoHints warning in infinite mode', () => {
    render(<Game {...createDefaultProps({ isCompetition: false })} />);
    expect(screen.queryByText('Enabling GeoHints will score half points')).not.toBeInTheDocument();
  });
  ...
  it('renders guesses when submitted', () => {
    const guesses: Guess[] = [{ country: 'Spain', correct: false, distance: 500, direction: 'NE' }];
