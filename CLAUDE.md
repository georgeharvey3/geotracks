...
- Each calendar day is seeded with the same ten Songs (`src/helpers/getDailySongs.ts`, Mulberry32 PRNG) so scores are comparable. **The seeded list is Competition's alone** (issue #49): `pickDailySong` is the only draw that reads it or walks `dailySongIndex`, and it is called from `START_RUN` and from Competition's `NEXT_SONG` and nowhere else. Every other draw — the Song the menu opens on, every Infinite round, the walk back to the menu — goes through `pickRandomSong` and leaves the day whole, so a player cannot be handed today's answers by playing Infinite first. Albums are removed from the pool after selection to prevent repeats within a session.
...
**Only Competition's answers are knowable in advance.** `answerAt(turn)` recomputes the day's seeded ten and is Competition's alone; Infinite draws at random (issue #49), so its rounds read the answer back from the Song the player was actually handed — `spotifyPlayerControl.loadedLink()` on the player fake, looked up in the library (`currentAnswer()`). No test may assert which Song Infinite plays.
