...
The **Library** is every Album the app holds, and it is **two files unioned by `src/music/library.ts`** — after issue #40 nothing else imports a raw album JSON. `gameReducer`, `exploreReducer` and the tests all take `library`; leaving each of them to concatenate would be four places to keep in step, and the failure mode when one is missed is Explore offering a country the game's pool has never heard of.

`src/albums.json` is the **Smithsonian Folkways** half, and the filename is the provenance record: every Album in it came from the Smithsonian Folkways Archive, catalogued by hand into six per-continent Google Sheets in 2023 and then looked up on Spotify one album at a time — 785 albums across 134 countries, edited by hand from here on. `src/community-albums.json` is the other half: **Community albums**, added one accepted Suggestion at a time by `scripts/add-community-album.ts`. Appending them to `albums.json` would make the Folkways claim quietly false and unseparable forever, which is the whole reason for the split. `Album` deliberately gains **no `source` field** — provenance is positional, and a runtime field would give every consumer of an Album (the daily seed, Explore's queue, the Run summary's rows) an optional property it must decide to ignore.

**`competitionAlbums(library, today)` and `liveFrom` are load-bearing.** A Community album joins the Library at once but joins **Competition** only once the day has passed the `liveFrom` date it carries, because `getDailySongs` draws _by array index_ and `createInitialState` re-derives the day's ten on every page load: adding one album mid-day would hand two players on the same day different Daily Songs onto the same leaderboard, and would hand a player resuming an unfinished Run six turns from a sequence their first four were never part of. `liveFrom` is a **switch-on date, not a provenance date**, authored the day _after_ the intended release so that nobody has passed it when the deploy lands and everyone crosses it at their own local midnight. The comparison takes the date as an **argument**, following `helpers/dailyRun.ts` — `vi.setSystemTime` moves the daily seed, and no test may assert which Songs a day yields. See ADR-0005.
...
player fills the form
  ...
  → node scripts/add-community-album.ts (lists them, you pick one)
  → src/community-albums.json           (committed)
  → src/music/library.ts unions it with albums.json
  → live in Explore + Infinite on deploy; live in Competition from `liveFrom`
```
...
- `src/music/library.ts` — The Library: the union of the two album files, and `competitionAlbums(library, today)`'s `liveFrom` gate
...
- `src/community-albums.json` — The same shape plus `liveFrom`, machine-written by the accept script. Small, so Prettier owns it: **not** `.prettierignore`d
