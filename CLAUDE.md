...
`.env` also holds `SPOTIFY_CLIENT_ID` / `SPOTIFY_CLIENT_SECRET`, which **are** secrets and are the only ones the repo has. They are deliberately **not** `VITE_`-prefixed — Vite injects every `VITE_*` variable into the client bundle, so a rename would publish the credential to every visitor. Nothing in `src/` reads them; they exist for `scripts/add-community-album.ts`.
...
**Two grounds, and which one a screen is on follows from its family.** The app pages (game, Explore, Run summary) are ink on cream. The **content pages** (menu, scoreboard, Suggest) stand on the **night backdrop** — the map under a black veil (`BackdropMap`) — and draw their chrome in paper: `paper` for type, `paperMuted` for secondary and for the wordmark's `Geo`, and a paper focus ring, scoped by `[data-surface="night"]` in `src/index.css`. Opaque surfaces inside them (the scoreboard's table card, the Suggestion form's) are MUI `Paper`, which resets to cream and ink on its own.
...
- `useSuggestions` — the Suggestion write: the same RTDB seam with its read half missing
- `useCommunityAlbums` — the live half of the Library, ordered by push key, reporting loading/ready/failed
- `useAdmin` — the review screen's other half: named sign-in, the Suggestions read, and the one delete in the app
...
- The same database holds `suggestions/` (see "Community suggestions"), which is the leaderboard's arrangement with the read half removed: same anonymous auth, same create-only per-record write, same strict `.validate`, but **no `.read`** — it inherits `false` from the root, so the only reader is the Firebase console. Both nodes share one Spark-tier database with a 1 GB cap, which is why the 500-character `note` is the one place in the database where a stranger can store content and why App Check is the named response to abuse.
...
- Each calendar day is seeded with the same ten Songs (`src/helpers/getDailySongs.ts`, Mulberry32 PRNG) so scores are comparable. **The seeded list is Competition's alone** (issue #49): `pickDailySong` is the only draw that takes a Song off it or walks `dailySongIndex`, and it is called from `START_RUN` and from Competition's `NEXT_SONG` and nowhere else. Every other draw — the Song the menu opens on, every Infinite round, the walk back to the menu — goes through `pickRandomSong`, so Infinite is never _served_ the day's Songs and never spends one. It draws from the same 785 Albums, so a round can still land on an Album from one of the day's answer countries by chance (~1 in 79); that is one library shared by two modes, not the day's list leaking. Albums are removed from the pool after selection to prevent repeats within a session.
...
- **Playable countries** — a country is Playable when the app holds at least one Album for it: 134 of 246 today (132 polygons, 15 of the 87 straggler markers). Non-playable countries take the inert-land fill and **still show their name on hover** — an absence of music is not an absence of geography. Since issue #40 they are **selectable too**, but picking one asks about it rather than plays it (see below). Shapes the app has no country for at all stay inert and differ only in having no name to show.
- **Fills** — flat states: inert land (non-playable), land (Playable), the near-white highlight (hover, and the country being asked about), and green for the country now playing. No rings or halos; Explore leaves the base map's `overlay` slot unused.
...
- **Asking about a silent country** — Explore has a **second kind of selection**: the country being listened to, and the country being _asked about_ (`askedAbout`, `ASK_ABOUT`). Clicking a non-playable country selects it as the latter — no navigation, no music stopped, nothing played — and the panel then offers "No music from X yet · Suggest an album". **The obvious version of this is a trap**: Explore passes `armOnTouch={false}` because choosing a country costs nothing but the Song now playing, so if a silent country navigated to the Suggestion form, one stray tap on Chad while listening to Mali would unmount the player and cost the song and the screen both. Going to the form is a deliberate second action — the same arm-then-commit shape the touch rule already uses, applied to the thing that now has a cost. `Explore.test.tsx` pins that clicking a silent country does not stop the music; without that assertion the whole arrangement is invisible.
- **Lifetime** — leaving Explore unmounts the player and therefore stops the music, and unchooses the country with it (`LEAVE`, which also drops the country being asked about), so coming back opens on the map in silence rather than resuming the Song mid-flight. The queues live in the provider above the screen and are kept: choosing that country again picks up where it left off, which is what stops it repeating itself. `LEAVE` is dispatched from a **layout**-effect cleanup — a dispatch from a passive cleanup while the subtree is being deleted never reaches the reducer. Nothing is persisted across a page reload — no storage, no schema to version.
...
- **`START_RUN` anchors the Run at the day's first Song** (`dailySongIndex` 0) whatever the session left the index at. It is now belt to a brace — nothing outside Competition moves that index ([#49](https://github.com/georgeharvey3/geotracks/issues/49), closed) — and it stays because the Run being the day's ten _from the top_ is the thing that makes scores comparable, and that should not rest on an invariant held somewhere else.

### Music library

The **Library** is every Album the app holds, and it is **a bundled file plus a live database node**, unioned by `src/music/library.ts` (`libraryWith`) — nothing else reads either source. `gameReducer` and `exploreReducer` take albums as a parameter, defaulting to the bundled half, and are topped up by an `ALBUMS_LOADED` action when the live half lands.

`src/albums.json` is the **Smithsonian Folkways** half, and the filename is the provenance record: every Album in it came from the Smithsonian Folkways Archive, catalogued by hand into six per-continent Google Sheets in 2023 and then looked up on Spotify one album at a time — 785 albums across 134 countries, edited by hand from here on. The other half is **`communityAlbums/` in RTDB**: Community albums, written one accepted Suggestion at a time by `scripts/add-community-album.ts` and live without a deploy ([ADR-0007](docs/adr/0007-community-albums-live-in-the-database.md)). The split is about **size and mutability, not trust** — the half that changes is small, and the half that is 0.91 MB never changes. Putting the community albums into `albums.json` would make the Folkways claim quietly false and unseparable forever; `Album` deliberately gains **no `source` field** — provenance is positional, and a runtime field would give every consumer of an Album (the daily seed, Explore's queue, the Run summary's rows) an optional property it must decide to ignore. There is **no `community-albums.json`**: it was deleted with ADR-0007, and the node is the only copy the community catalogue has.

**`competitionAlbums(albums, today)` and `liveFrom` are load-bearing, and more so now the pool is live.** A Community album joins Explore and Infinite the moment it is accepted, and joins **Competition** only once the day has passed the `liveFrom` it carries. `getDailySongs` draws _by array index_ and `createInitialState` re-derives the day's ten on every page load, so what the seed is owed is narrow and absolute: **the same set, in the same order, for every player on that date, and no change once the date has begun.** A bundled file gets that by being immutable between deploys; a database record gets it from this gate. `liveFrom` is a **switch-on date, not a provenance date**, authored a day ahead so nobody anywhere has passed it when it is written, and everyone crosses it at their own local midnight. The comparison takes the date as an **argument**, following `helpers/dailyRun.ts` — `vi.setSystemTime` moves the daily seed, and no test may assert which Songs a day yields. See ADR-0005 and ADR-0007.

**An album whose `liveFrom` has passed is frozen.** Nothing in the code can enforce this: editing or deleting one changes the pool's length and order for anyone who loads afterwards, and hands them a different day. A mistake in a live album is corrected on a _future_ `liveFrom`, not in place.

**Competition fails closed on the live read** — the one place in the app that does. `useCommunityAlbums` reports `loading` / `ready` / `failed`, `GameProvider` surfaces it as `libraryStatus`, and the menu's Competition button is disabled until it is `ready`. A Run seeded from a half-arrived pool would play a different ten onto the same leaderboard with nothing looking wrong, which is worse than not starting. The Daily Run's stored record fails the other way on purpose, because a serialization bug of ours must not look like a punishment. Explore and Infinite never wait.

The coverage review (issue #41) is closed and concerns `albums.json` alone. It reconciled the library against the sheets and added the 125 albums that pass had never reached; the twenty it could not resolve — mostly untitled-by-year entries like `China`, `Croatia` and `One Sky` — were left out, and every country they belong to is Playable from another album regardless. The tooling that did it has been removed; it is in the history if it is ever wanted again.

### Community suggestions

A player can propose an Album for the Library — a **Suggestion**: the album on Spotify, the country they say it belongs to, and optionally why (issue #40, [ADR-0005](docs/adr/0005-suggestions-collected-in-the-app-and-accepted-by-a-commit.md)). **The app collects Suggestions; it does not act on them.** What becomes dynamic is the collecting — accepting one is still a commit, a PR into `develop`, a release merge and a deploy.

The chain:

```
player fills the form
  → suggestions/{pushId} in RTDB        (write-only from the client)
  → reviewed at #admin, or listed in the terminal
  → node scripts/add-community-album.ts (one atomic update:
  →                                      writes the album, deletes the Suggestion)
  → communityAlbums/{pushId} in RTDB
  → src/music/library.ts unions it with albums.json
  → live in Explore + Infinite at once; live in Competition from `liveFrom`
```

- **The screen** is the sixth `Screen` value, in the content-page family — night backdrop, paper chrome, the form inside an MUI `Paper` card. **Not a modal**: the app has no `Dialog`/`Modal`/`Snackbar` anywhere and `design.md` is locked, so one would have to be designed (scrim, focus trap, dismissal, its argument with the lockup's flight). `screen` gains a value and a **prefill** (`suggestCountryCode`, carried by `SHOW_SUGGEST` as `RESUME_RUN` carries a record) and nothing else about a Suggestion goes in the game reducer — the form's own state is local, and the write goes through `useSuggestions`.
- **Two ways in**: a second outlined secondary button on the menu (never a fourth primary — see `design.md`'s CTA voice), and Explore's panel offer for the country being asked about.
- **The form asks only for what Spotify cannot tell you.** The accept script fetches the album name, artist and every track URL from the id; what it can never fetch is the country or the reason. So it must **never ask for an album name** — a field the submitter can get wrong about something the machine knows for certain. A `/track/` link is rejected **by name** ("That's a track link — paste the album instead"), because it is the overwhelmingly likely wrong paste; resolving track links to albums was refused, since a track id and an album id are both 22 base62 characters and the stored record would carry an ambiguity no `.validate` rule could settle.
- **The record** is `suggestions/{pushId}: { albumId, countryCode, note?, uid, createdAt }`. The **album id, not the URL**, because every Spotify share link carries a `?si=` tracking token and the URL is trivially rebuilt; the **alpha-2 code, not the name**, because a Suggestion may sit unreviewed for weeks and "is this string one of 246 names?" is not expressible in an RTDB rule. `uid` is kept — anonymous auth mints one anyway, and a public-write node with no attributable identity can only be defended by deleting all of it. There is **no contact field and no name**: an "email (optional)" box that never receives a reply is worse than no box.
- **The node is write-only for players.** `suggestions` grants `.read` to exactly one uid in `database.rules.json` — the reviewer's (see "Reviewing Suggestions") — so every player writes and never reads. The Firebase console and the owner's Firebase CLI read it too, by the same owner privilege, which is what the accept script goes through. Abuse is met with a submit-button guard for now; **if junk appears, the response is Firebase App Check (reCAPTCHA v3)** — decided in advance so it is executed rather than invented in a panic.
- **`CountryInput` split for this.** It owns its own `<form>`, clears itself on submit and ends in a Send button, so it is a _guess submitter_, not a controlled field. The picking is now `src/Components/CountryAutocomplete/CountryAutocomplete.tsx` (`value`/`onChange`, no form, no button) and `CountryInput` is a thin wrapper over it: one picker in the app, two callers. Free-text country entry is the easiest way to break the name join ("Ivory Coast", "USA"), so the picker is not optional in the form either.
- **The accept script is a transcription tool, not a gatekeeper** — nothing can verify the country makes sense; that is the human review step. Run bare, it **lists the waiting Suggestions and accepts the one you pick**; naming `<album-url-or-id> <ALPHA2>` adds an album nobody suggested. It **reads and writes the database through the Firebase CLI** (already installed, already logged in, `.firebaserc` names the project), so there is still no admin SDK and no service-account JSON on disk. Accepting is one **atomic multi-path update**: the album lands in `communityAlbums/` and the Suggestion is deleted, so the queue empties itself and there is no "reviewed" flag. The `suggestion` push key is still recorded on the album, now as provenance rather than bookkeeping. It catches duplicates by **track URL** because `albums.json` holds no album ids at all — across the bundled file and the live node both — strips query params, strips control characters out of the `note` before printing it (a stranger wrote it, and a terminal takes escape sequences as instructions), writes the country **name** (the join is by name), defaults `liveFrom` to tomorrow, and takes `--dry-run`. It is `.ts` rather than `.mjs` so it can import `extractAlbumId` from `src/helpers/spotifyAlbum.ts` — which is why that helper is **import-free** and the import carries an explicit `.ts` extension.

**Out of scope, deliberately:** email notification (the Firebase "Trigger Email" extension, if the box fills faster than it is checked), App Check, contributor credit (`suggestedBy` can be added to our own file later without the form having asked), and any notion of the Library updating without a deploy. In-app review was out of scope too and no longer is — see below.

### Reviewing Suggestions

The **review screen** ([ADR-0006](docs/adr/0006-suggestions-reviewed-in-app-behind-a-google-account.md)) is where a Suggestion is judged: artwork, title, country, note and a Spotify link, in a queue. The terminal already lists and accepts them, so this exists for the one thing a terminal cannot do — **show you the record**. Deciding whether an album belongs in a music app on the strength of a 22-character id is not deciding. It supersedes ADR-0005's "no in-app review or moderation UI"; **accepting is still a commit**, which is the part that has not moved.

- **The gate is a Google account, not a token.** A password in `.env` cannot work on a static build: anything the bundle compares against is in the bundle, and a `VITE_` prefix would publish it outright. So `database.rules.json` names the one uid allowed to read `suggestions` and Firebase enforces it. The uid is **committed in a public repo** — a uid is not a secret — the screen **ships to everyone** and is empty for everyone else, and **nothing in `src/` knows which uid it is**: `useAdmin` subscribes and finds out, and `denied` is what `PERMISSION_DENIED` is called on the way back. Legacy Firebase _database secrets_ (full admin via a query param) and custom claims (which need a service-account JSON on disk) were both rejected.
- **`#admin` is the only way in** — no menu button, no link. It is the app's one piece of URL awareness, read once at mount and cleared on the way out, and it is a **convenience, not a boundary**: the hash is guessable and meant to be irrelevant.
- **Reject deletes; accept copies a command.** Rejecting is the app's only destructive write, on a second `.write` branch for the admin uid (the players' create-only branch is untouched). It arms then commits — two clicks, not a `Dialog`, which the app does not have. Accepting ends in a commit no browser can make, so the button hands over `node scripts/add-community-album.ts <id> <CODE>`.
- **Album metadata is oEmbed**, public and credential-free, as `useSpotifyPlayer` already uses for Songs — the catalogue API needs the client secret, which must never reach a browser. A row whose album will not resolve keeps its country, note and link.
- **What it costs to set up:** the Google provider enabled in the Firebase console, and the uid pasted into `database.rules.json` and deployed. Until then the screen denies everyone including its author, which is why the denied state **prints the uid** — the first sign-in is always denied, and is what bootstraps the rule.
...
- `src/state/exploreReducer.ts` — Pure Explore reducer: Playable countries, the country being listened to, the country being asked about, and each Country queue, plus the `currentSong()` selector
...
- `src/hooks/` — Side-effect seams: `useSpotifyPlayer`, `useKeyboardShortcuts`, `useLeaderboard`, `useSuggestions`, `useDailyRun`, `useHasHover`
...
- `src/helpers/spotifyAlbum.ts` — `extractAlbumId`: the Suggestion form's whole link validation, and the accept script's. **Import-free**, because the script runs under bare node
- `src/music/library.ts` — The Library: `bundledAlbums`, `libraryWith(live)` and `competitionAlbums(albums, today)`'s `liveFrom` gate
...
- `src/Components/SuggestScreen/SuggestScreen.tsx` — The Suggestion screen's container: the RTDB write wired to the form
- `src/Components/AdminScreen/AdminScreen.tsx` — The review screen's container: the read/reject seam wired to the queue
- `src/Components/Admin/Admin.tsx` — The review queue and its four states (signed out, checking, denied, allowed)
- `src/Components/SuggestionRow/SuggestionRow.tsx` — One Suggestion, as something to decide about
- `src/Components/Suggest/Suggest.tsx` — The Suggestion form itself, and its four states (editing, sending, sent, failed)
- `src/Components/CountryAutocomplete/CountryAutocomplete.tsx` — The app's one country picker, controlled. `CountryInput` wraps it for guessing
...
**Only Competition's answers are knowable in advance.** `answerAt(turn)` recomputes the Daily Songs and is Competition's alone; Infinite draws at random (issue #49), so its rounds read the answer back from the Song the player was actually handed — `spotifyPlayerControl.loadedLink()` on the player fake, looked up in the library (`currentAnswer()`). No test may assert which Song Infinite plays.
...
Community suggestions (issue #40) added **one** mocking boundary, which the repo should stay proud of: the RTDB write, behind `useSuggestions`, with `src/test/suggestionFake.ts` beside `leaderboardFake.ts` — it records what was submitted and can be told to fail, which is the only way the form's retryable failure state is reachable without a network. Everything else the feature brought is pure and needs nothing: the Library union and the `liveFrom` gate are covered by `src/music/library.test.ts` with fixtures and no clock faking, and `extractAlbumId` by `src/helpers/spotifyAlbum.test.ts`. **The accept script has no test**, as `build-map-geometry.mjs` and `build-logo-assets.ts` have none — its one piece of interesting logic is that shared, tested helper. Two notes for anyone adding to the form's tests: the picker's suggestions are found by **text content** rather than by accessible name (their label is split across a `<strong>`, as `CountryInput.test.tsx` already works around), and `Explore.test.tsx`'s "does not stop the music when it is picked mid-Song" is the whole reason the map click does not navigate — it is invisible unless a test says so.
