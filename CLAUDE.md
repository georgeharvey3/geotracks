...
- `node scripts/add-community-album.ts <album-url-or-id> <ALPHA2> [--live-from YYYY-MM-DD] [--dry-run]` — Accept a Suggestion into `src/community-albums.json` (needs Spotify credentials in `.env`; see "Community suggestions")
...
player fills the form
  ...
  → read in the Firebase console
  → node scripts/add-community-album.ts <album-url> <ALPHA2>
...
- **The node is write-only from the client.** `suggestions` gets no `.read` in `database.rules.json` and inherits `false` from the root, so the only reader is the Firebase console. Abuse is met with a submit-button guard for now; **if junk appears, the response is Firebase App Check (reCAPTCHA v3)** — decided in advance so it is executed rather than invented in a panic.
...
- **The accept script is a transcription tool, not a gatekeeper** — nothing can verify the country makes sense; that is the human review step. It **does not touch the database** (no admin SDK, no service-account JSON, no write-back, no "reviewed" flag), catches duplicates by **track URL** because `albums.json` holds no album ids at all, strips query params, writes the country **name** (the join is by name), defaults `liveFrom` to tomorrow, and takes `--dry-run`. It is `.ts` rather than `.mjs` so it can import `extractAlbumId` from `src/helpers/spotifyAlbum.ts` — which is why that helper is **import-free** and the import carries an explicit `.ts` extension.
